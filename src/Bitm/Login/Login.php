...
//Prepare Connection
  ...
  <strong>Error!</strong> Data has not been stored successfully.
    ...
    {
        $query = "SELECT id,user_id,email,password,role,status,district_cd FROM `user_info`   WHERE `email`='".$this->email."'  AND `password`='".$this->password."'";//'".$this->password."'  AND `password`='1'
        //echo $query;
        //die();
        $result = mysqli_query($this->conn, $query);
