...
for($i=1;$i<=$noOfPage;$i++){
  ...
  $pagination.="<li class='$active'><a href='user_approval.php?pageNumber=$i'>$i</a></li>";
}

$pageStartFrom=$itemPerPage*($pageNo-1);

$allUser=$uApproval->paginator($pageStartFrom,$itemPerPage);
//Utility::dd($allUser);
//die();
$prev=$pageNo-1;
$next=$pageNo+1;
  ...
  <div class="content">
      ...
      <div class="row">
        ...
        <div class="col-lg-12 col-md-10 col-sm-10 col-xs-10">
          <div class=""><!-- ebox-->
            ...
            <form role="form" action="../User/user_approval.php">
    ...
    <thead>
      <tr class="">
        ...
        <th data-filterable="true" data-sortable="true">Tel No</th>
        <th data-filterable="true" data-sortable="true">SignUp Date</th>
    ...
    foreach($allUser as $data){
      ...
      <tr class="">
        ...
        <td width=""><?php echo $data['phone_no']?></td>
        <td width=""><?php echo date('d-M-Y',strtotime($data['signup_date']))?></td>
        <td width=""><?php echo $data['district_name'] ?></td>

        <td width="15%">
          <?php
          if ($status==0) {
            //$deleted_at="<a href=\"../Visitor_Message/trash_messsage.php?id=".$idd."\"class=\"btn btn-info btn-xs\" role='button'>Unread</a>";
            $userStatus= "<a href=\"../Neci/updateUser.php?id=".$uid."\"class=\"btn btn-warning btn-xs\" role='button'>Pending</a>";
          } else {
            $userStatus= "<a href=\"../Neci/updateUser.php?id=".$uid."\" class=\"btn btn-info btn-xs\" role='button'>Approved</a>";
          }
          ?>

          <?php echo $userStatus?>
          <a href="../Neci/deleteUser.php?id=<?php echo $data['id']?>" class="btn btn-primary  btn-xs" role="button">Delete</a>
        </td>
      </tr>
    <?php } ?>
      ...
      <ul class="pagination">
        <?php if($pageNo>1) {echo "<li><a href='../User/user_approval.php?pageNumber=$prev'>Prev</a></li>";}else{echo "";}?>
        <?php echo $pagination?>
        <?php if($pageNo<$noOfPage){echo "<li><a href='../User/user_approval.php?pageNumber=$next'>Next</a></li>";}else{echo "";}?>
      </ul>
